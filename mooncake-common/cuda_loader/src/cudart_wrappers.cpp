...
// ---------- Batch memcpy (CUDA 12.8+) ----------

#if CUDART_VERSION >= 13000
cudaError_t cudaMemcpyBatchAsync(const void** dsts, const void** srcs,
                                 const size_t* sizes, size_t count,
                                 const cudaMemcpyAttributes* attrs,
                                 const size_t* attrsIdxs, size_t numAttrs,
                                 cudaStream_t stream) {
    auto& s = GetCudaRtSymbols();
    if (s.p_cudaMemcpyBatchAsync)
        return s.p_cudaMemcpyBatchAsync(dsts, srcs, sizes, count, attrs,
                                        attrsIdxs, numAttrs, stream);
    return cudaErrorNoDevice;
}
#elif CUDART_VERSION >= 12080
cudaError_t cudaMemcpyBatchAsync(void** dsts, void** srcs, size_t* sizes,
