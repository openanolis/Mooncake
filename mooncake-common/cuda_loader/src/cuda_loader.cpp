...
class CudaLoader {
   ...
   private:
    ...
    void loadCudaRt() {
        ...
        // Optional: batch memcpy (CUDA 12.8+), not required for basic
        // operation
#if CUDART_VERSION >= 12080
        if (!LoadSymbol(cudart_handle_, "cudaMemcpyBatchAsync",
                        rt_.p_cudaMemcpyBatchAsync)) {
            LOG(INFO)
                << "cuda_loader: cudaMemcpyBatchAsync not available "
                << "(requires CUDA 12.8+), batch memcpy will use fallback";
        }
#endif
