...
backend on top of the existing Store interfaces, uses batched metadata lookups,
and caches successful query results until their leases expire. It still does
not depend on:
...
2. fetch or refresh one batched set of replica metadata with `batch_query(...)`
3. reuse cached query results until the lease expires or the layer is mutated
4. fetch per-head tables with `batch_get_buffer_with_query_results(...)`
...
Populate and remove both invalidate the cached query metadata so the next
lookup always re-resolves the layer after a mutation.
...
- cache invalidation coverage in `scripts/test_engram.py`
