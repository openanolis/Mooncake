...
|---|---|---|
| `MOONCAKE_NVME_KV_DEVICE_PATH` | unset | Default NVMe character device path used by all NVMe KV device IDs, for example `/dev/ng0n1`. |
| `MOONCAKE_NVME_KV_DEVICE_PATH_<device_id>` | unset | Per-device override. `<device_id>` must contain only letters, digits, `_`, `-`, or `.` and must not be `.` or `..`. |
| `MOONCAKE_NVME_KV_DEVICE_IDS` | `default` | Comma-separated logical device IDs used for placement. |
| `MOONCAKE_NVME_KV_NSID` | `1` | NVMe namespace identifier submitted in ioctl commands. |
