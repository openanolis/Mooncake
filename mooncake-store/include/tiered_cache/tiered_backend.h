...
public:
    ...
    bool Put(const std::string& key, uint64_t target_tier_id, const DataSource& source);
    bool Delete(const std::string& key);
    bool MoveData(const std::string& key, uint64_t src_tier_id, uint64_t dest_tier_id);
