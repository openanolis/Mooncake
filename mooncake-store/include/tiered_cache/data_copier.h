 ...
 * its copy functions to and from DRAM *must* be provided via the CopierRegistry.
 */
...
public:
     ...
     */
    DataCopierBuilder& AddDirectPath(MemoryType src_type, MemoryType dest_type, CopyFunction func);
...
public:
     ...
     * @param dest_ptr A pointer to the destination (memory address, handle, etc.).
     * @return True if the copy was successful, false otherwise.
     */
    bool Copy(const DataSource& src, MemoryType dest_type, void* dest_ptr) const;

private:
    friend class DataCopierBuilder; // Allow builder to access the constructor.
    DataCopier(std::map<std::pair<MemoryType, MemoryType>, CopyFunction> copy_matrix);

    CopyFunction FindCopier(MemoryType src_type, MemoryType dest_type) const;
    const std::map<std::pair<MemoryType, MemoryType>, CopyFunction> copy_matrix_;
};
