...
public:
     ...
     */
    void RegisterMemoryType(MemoryType type, CopyFunction to_dram, CopyFunction from_dram);
    ...
    // These methods are used by the DataCopierBuilder to collect all registrations.
    const std::vector<MemoryTypeRegistration>& GetMemoryTypeRegistrations() const;
    const std::vector<DirectPathRegistration>& GetDirectPathRegistrations() const;
...
/**
 * @brief A helper class to automatically register copiers at static initialization time.
 *
 * To register a new memory type, simply declare a static instance of this class
 * in the corresponding .cpp file, providing the type and its to/from DRAM copiers.
 */
