 ...
 */
struct DataSource {
    const void* ptr;  // Pointer to the data. Its interpretation depends on the `type`.
 ...
 *
 * This class defines the common interface that all storage media (DRAM, VRAM, SSD, etc.)
 * must implement. The interface is designed to be simple and focuses on the
 * essential operations of a storage layer, leaving complex eviction and promotion
 * logic to the TieredBackend and CacheScheduler.
 */
class CacheTier {
public:
     ...
     * @param data [out] A reference to a void pointer that will be set to the data's location.
     ...
     * This is used to describe the data as a source for a subsequent copy operation
     * when it needs to be moved to another tier.
     * @param key The key to describe.
     * @return A DataSource object. If the key is not found, the `ptr` member will be null.
     */
...
protected:
    // A pointer to the parent backend, allowing tiers to access shared services like the DataCopier.
