...
bool TieredBackend::Init(Json::Value root, TransferEngine* engine) {
    ...
    LOG(INFO) << "TieredBackend initialized successfully with " << tiers_.size() << " tiers.";
    return true;
}
...
bool TieredBackend::Put(const std::string& key, uint64_t target_tier_id, const DataSource& source) {
...
bool TieredBackend::MoveData(const std::string& key, uint64_t src_tier_id, uint64_t dest_tier_id) {
    VLOG(1) << "Moving key '" << key << "' from tier " << src_tier_id << " to " << dest_tier_id;

    auto src_it = tiers_.find(src_tier_id);
    auto dest_it = tiers_.find(dest_tier_id);
    if (src_it == tiers_.end() || dest_it == tiers_.end()) {
        LOG(ERROR) << "MoveData failed: Invalid tier ID. Source: " << src_tier_id << ", Dest: " << dest_tier_id;
        return false;
    }
    ...
    if (key_it == key_to_tier_map_.end() || key_it->second != src_tier_id) {
        LOG(WARNING) << "MoveData failed: Key '" << key << "' is not in the expected source tier " << src_tier_id;
        return false;
    }

    DataSource source = src_tier->AsDataSource(key);
    if (source.ptr == nullptr) {
        LOG(WARNING) << "Key '" << key << "' disappeared from tier " << src_tier_id << " during move operation.";
        return false;
    }

    if (!dest_tier->Put(key, source)) {
        LOG(WARNING) << "Could not move key '" << key << "' to destination tier " << dest_tier_id << " (likely full).";
        return false;
    }
