...
std::optional<ScatterReadHandle> RealClient::streaming_batch_get_buffer_ranges(
    ...
    if (keys.size() != dest_offsets.size() ||
        ...
        LOG(ERROR) << "streaming_batch_get_buffer_ranges: size mismatch";
        return std::nullopt;
    }
    if (n == 0) {
        LOG(ERROR) << "streaming_batch_get_buffer_ranges: empty input";
        return std::nullopt;
    }

    // De-duplicate keys for BatchQuery
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (!replica.is_memory_replica()) {
            LOG(ERROR) << "streaming_batch_get_buffer_ranges: disk replicas "
                          "not supported";
            continue;
        }
    ...
    if (key_ranges_map.empty()) {
        LOG(ERROR) << "streaming_batch_get_buffer_ranges: no valid ranges";
        return std::nullopt;
    }
    ...
    return client_->StreamingBatchTransferReadRanges(dest_buffer, key_ranges);
}
