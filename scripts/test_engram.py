...
class TestStorePopulateAndLookup(EngramTestBase):
    ...
    def test_repopulate_invalidates_cached_query_results(self):
        _, engram = self.create_engram()
        self.populate_store(engram)

        row_ids = np.array([[[1, 2, 3, 4]]], dtype=np.int64)
        first_lookup = np.asarray(engram.lookup(row_ids))

        self.assertEqual(engram.remove_from_store(force=True), engram.get_num_heads())

        replacement_tables = []
        for head_idx, vocab_size in enumerate(engram.get_table_vocab_sizes()):
            replacement_tables.append(
                np.full(
                    (vocab_size, engram.get_embedding_dim()),
                    fill_value=10000.0 + head_idx,
                    dtype=np.float32,
                )
            )
        engram.populate(replacement_tables)

        second_lookup = np.asarray(engram.lookup(row_ids))
        self.assertFalse(np.array_equal(first_lookup, second_lookup))
        for head in range(engram.get_num_heads()):
            idx = row_ids[0, 0, head]
            np.testing.assert_allclose(
                second_lookup[0, 0, head], replacement_tables[head][idx]
            )
